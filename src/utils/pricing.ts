import { ModelInfo, ModelResult } from '../core/types';
...
export class PricingCalculator {
  ...
  static calculatePricing(
    ...
    const blendedCostPerMillion = totalTokens > 0 ? (actualCost / totalTokens) * 1_000_000 : 0;
  ...
  static formatActualCost(cost: number): string {
    ...
    if (cost < 0.00001) return `$${cost.toFixed(8)}`; // Show more decimals for very small costs
    if (cost < 0.001) return `$${cost.toFixed(6)}`;
    if (cost < 0.1) return `$${cost.toFixed(5)}`;
  ...
  static formatCostPerMillion(costPerMillion: number): string {
    if (costPerMillion === 0) return '$0.00/M';
